#PREPROCESSING FOR INITIAL DATASET--------
...
colnames(language_df) <- target_languaFOR BIG ges
...
for (col_name in names(data_scientist)) {
  p <- ggplot(data_scientist, aes_string(x = col_name)) +
    geom_bar(fill = "skyblue", color = "black") +
    labs(title = paste("Occorrenze per", col_name),
         x = col_name,
         y = "Frequenza") +
    theme_minimal() +
